...
- Edge-секреты: `TELEGRAM_BOT_TOKEN`, `OPENAI_API_KEY`, `ENCRYPTION_SECRET_HEX` (64 hex), `MINI_APP_BASE_URL`, `SUPABASE_JWT_SECRET` (нужен `auth-telegram` для подписи JWT; ставить вручную через `supabase secrets set` — автоматически НЕ инъектится), плюс автоматические `SUPABASE_URL` / `SUPABASE_SERVICE_ROLE_KEY`.
